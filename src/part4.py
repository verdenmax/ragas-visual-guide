...
<div class="card detail">
  ...
  经典栈与两个枚举都在 <span class="mono">src/ragas/metrics/base.py</span>：<span class="mono">Metric</span> / <span class="mono">MetricType</span> / <span class="mono">MetricOutputType</span> / <span class="mono">MetricWithLLM</span> / <span class="mono">MetricWithEmbeddings</span> / <span class="mono">SingleTurnMetric</span> / <span class="mono">MultiTurnMetric</span>，以及现代根基类 <span class="mono">SimpleBaseMetric</span> / <span class="mono">SimpleLLMMetric</span>、协议 <span class="mono">ModeMetric</span> 和投票器 <span class="mono">Ensember</span>；<span class="mono">required_columns</span> 属性与 <span class="mono">get_required_columns</span> 也在此。
...
row.model_dump_json()   <span class="cm"># {"score": {"value": 0.83, "reason": "…"}}  走 __json__</span></pre>
...
<div class="card spark">
  ...
  <ul>
    ...
    <li><strong>双口径序列化</strong>：<span class="inline">model_dump()</span> 留对象、<span class="inline">model_dump_json()</span> 转字典，既方便内存里继续用，又方便落盘。</li>
  </ul>
</div>
...
<p><span class="inline">mode</span> 默认 <span class="mono">"f1"</span>、<span class="inline">beta</span> 默认 <span class="mono">1.0</span>（<span class="mono">beta&gt;1</span> 偏召回、<span class="mono">&lt;1</span> 偏精确）。precision 只做正方向、不算 <span class="mono">fn</span>；recall / f1 才跑双向。它在自己的 <span class="mono">util.py</span> 里定义了同名的 <span class="inline">NLIStatementPrompt</span>（与 Faithfulness 同范式、各自一份）。</p>
...
<span class="cm"># 输出 AspectCriticOutput(reason, verdict 0/1)；strictness>1 时多次采样取多数（Counter）</span></pre>
<p><span class="inline">AspectCritic(MetricWithLLM, SingleTurnMetric, MultiTurnMetric)</span> 同时支持单 / 多轮（两套 <span class="mono">PydanticPrompt</span>），<span class="inline">strictness</span> 默认 1 且会被强制成奇数（便于投票）。库里预置了 <span class="mono">harmfulness</span> / <span class="mono">maliciousness</span> / <span class="mono">coherence</span> / <span class="mono">correctness</span> / <span class="mono">conciseness</span> 几个现成裁判。</p>
