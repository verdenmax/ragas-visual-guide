...
QUIZZES = {
    "01-what-is-ragas.html": {
        "mcq": [
            ...
            {
                "q": "ragas 自己也大量用 LLM 当裁判——可 LLM 打分同样会「看心情」浮动。那它凭什么比「我觉得还行」更可靠？",
                "opts": [
                    "因为改用 LLM 打分后评分就完全客观，不再受任何主观因素影响",
                    "靠固定数据集 + 同一套 prompt + 可复现的实验记录，把波动锁住、让分数能横向对比和回归",
                    "因为 ragas 选用的裁判模型比人聪明，判断天生更准",
                    "因为 LLM 裁判每次都给出一模一样的分数，天然可复现",
                ],
                "answer": 1,
                "why": "关键不是「换了个更聪明 / 更客观的裁判」——LLM 裁判本身一样主观、会波动。ragas 的可靠来自把评测装进标准化框架：同一份数据集、同一套 prompt、可追溯的实验记录，于是分数可比较、可复现、可回归。vibe check 缺的正是这套框架，而不是缺一个更强的裁判。",
            },
        ],
        "open": [
        ...
        "mcq": [
            {
                ...
                "opts": [
                    ...
                    "为了把失败行直接记 0 分，对系统从严打分",
                    "因为 <code>np.nan</code> 会触发 ragas 自动换一个备用 LLM 重算该行",
                ],
                "answer": 1,
                "why": "默认 <code>raise_exceptions=False</code>，单行失败记 <code>np.nan</code>；汇总时 <code>safe_nanmean</code> 会忽略 <code>NaN</code>，所以个别失败行不会拖垮整批，也不会污染均值。注意它既不是记 0 分（那会拉低均值），也不是用均值回填。",
            },
            {
                ...
                "opts": [
                    ...
                    "<code>evaluate()</code> 才是真实现，<code>aevaluate()</code> 只是给它套了层 async 包装",
                    "<code>evaluate()</code> 是同步的，所以比 <code>aevaluate()</code> 更快",
                ],
                "answer": 1,
                "why": "async-first 设计：真正并发干活的是 <code>aevaluate()</code>，<code>evaluate()</code> 只套一层同步外壳（必要时借 <code>nest_asyncio</code> 兼容 Jupyter）——方向别记反。二者都会发 <code>DeprecationWarning</code> 指向 <code>@experiment</code>。",
            },
        ],
        "open": [
    ...
    "04-single-turn-sample.html": {
        "mcq": [
            ...
            {
                "q": "你建了个 <code>SingleTurnSample</code>，<code>reference</code> 忘了填（保持默认 <code>None</code>）。之后某指标按 <code>get_features()</code> 检查它要的列在不在——会怎样？",
                "opts": [
                    "字段虽是 <code>None</code> 但仍存在，<code>get_features()</code> 照样列出它，校验通过",
                    "<code>to_dict()</code> 用 <code>exclude_none=True</code> 丢掉了 None 字段，<code>reference</code> 不出现在 features 里，缺列校验直接报错",
                    "ragas 会在校验时用 LLM 自动把缺失的 <code>reference</code> 补上再继续",
                    "<code>None</code> 被当成空字符串参与计算，指标照常运行只是分数偏低",
                ],
                "answer": 1,
                "why": "<code>to_dict()</code> 即 <code>model_dump(exclude_none=True)</code>，<code>get_features()</code> 只返回它的 key——所以 None 字段会「凭空消失」，而不是以 None 形式留在 features 里。指标缺列校验看的正是 <code>get_features()</code>，于是忘填 <code>reference</code> 会被判成「缺这一列」直接报错。这正是「字段全 Optional + 按需带列」的另一面。",
            },
        ],
    },
        ...
        "mcq": [
            ...
            },
            {
                ...
                "opts": [
                    ...
                    "因为 parser 内置了各模型单价，传了它 ragas 才知道怎么把 token 换算成钱",
                    "因为只有显式传 parser，结果对象才会在内存里保留每行原始响应供事后统计",
                ],
                "answer": 1,
                "why": "各家模型返回 token 用量的字段格式不同，<code>token_usage_parser</code> 只负责「从回包里抽出用量数字」；没配它连 token 数都拿不到，<code>total_tokens</code> / <code>total_cost</code> 会抛「未配置成本计算」。注意它不管单价——把 token 换算成钱要另配每千 token 价格。",
            },
        ],
    },
    "08-experiment.html": {
        "mcq": [
            {
                "q": "<code>arun</code> 并发跑整张表时，某一行的任务抛了异常。这一行最终会怎样？",
                "opts": [
                    "整轮 <code>arun</code> 立即中断，连已跑完的行也不保存",
                    "异常被 <code>try/except</code> 捕获、打印一条 warning，这一行被跳过（不写入结果），其余行照常跑完并存盘",
                    "和 <code>evaluate()</code> 一样，这一行以 <code>np.nan</code> 占位写进结果后继续",
                    "ragas 自动把这一行重试三次，仍失败才记为错误",
                ],
                "answer": 1,
                "why": "<code>arun</code> 在 <code>asyncio.as_completed</code> 循环里逐个 <code>await</code>，单行抛错被 <code>try/except</code> 捕获、<code>print</code> 一条 warning 后继续——失败行根本不会 <code>append</code> 进 <code>Experiment</code>，最后 <code>save()</code> 只落成功的行。注意它和 <code>evaluate()</code> 的差别：后者失败记 <code>np.nan</code> 占位、行还在，<code>arun</code> 则直接「少一行」。",
            },
            {
                "q": "团队约定「跑实验前先 <code>version_experiment</code> 存档」。关于它到底做了什么，下列哪条准确？",
                "opts": [
                    "它会自动 <code>git push</code> 到远程，让队友立刻能拉到这次实验的代码",
                    "它只在本地提交一次并建 <code>ragas/{name}</code> 分支、返回 commit hash，默认仅暂存已跟踪文件，不碰远程",
                    "它新建一个独立 git 仓库专门存实验，与项目仓库隔离",
                    "它把数据集和实验结果一并提交进 git，实现数据版本化",
                ],
                "answer": 1,
                "why": "<code>version_experiment</code> 全程本地：<code>git add -u</code>（默认只暂存已跟踪文件，<code>stage_all=True</code> 才连未跟踪一起）→ 提交一次 → 建本地分支 <code>ragas/{name}</code> → 返回 commit hash。它不会 push，要同步给队友得自己推；它钉的是「代码快照」，实验结果另由 backend 落盘。",
            },
        ],
        "open": [
    ...
    "09-metrics-overview.html": {
        "mcq": [
            ...
            {
                "q": "你记住了「具名指标都从 <code>ragas.metrics.collections</code> 导入」，于是写下 <code>from ragas.metrics.collections import AspectCritic</code>，结果会怎样？",
                "opts": [
                    "正常导入，AspectCritic 和其他具名指标一样早已迁进 collections",
                    "报 <code>ImportError</code>：AspectCritic 是例外，collections 暂未提供，目前只能 <code>from ragas.metrics import AspectCritic</code>（会告警）",
                    "能导入，但它其实是空壳，运行 <code>ascore</code> 时才报错",
                    "导入成功且不告警，因为 collections 里的指标都不发弃用警告",
                ],
                "answer": 1,
                "why": "「具名指标走 collections」有例外：<code>AspectCritic</code>（及检索类的非 LLM 版）目前只在 legacy <code>ragas.metrics</code>，collections 的 <code>__all__</code> 并不含它。有意思的是，从 <code>ragas.metrics</code> 导它仍会发「请改用 collections」的告警——可 collections 偏偏还没有，于是这条告警对 AspectCritic 暂时「无处可去」，照旧用 legacy 即可。",
            },
        ],
    },
    "10-rag-metrics.html": {
        "mcq": [
            ...
            {
                "q": "线上 RAG 答案频繁「编造」——检索到的资料里明明没有，模型却自信地说出来。四件套里最该盯这个问题的是哪个？",
                "opts": [
                    "ContextPrecision：检索结果的排序质量",
                    "Faithfulness：答案是否「只依据检索到的上下文」、有没有无中生有",
                    "ContextRecall：该召回的资料是否都召回了",
                    "AnswerRelevancy：答案是否切题",
                ],
                "answer": 1,
                "why": "「编造」是生成端的 hallucination：检索其实给了对的资料，是模型没忠于它。<code>Faithfulness</code> 正是把答案拆成原子陈述、逐条核对能否由 <code>retrieved_contexts</code> 推出——别被名字带「检索」字样的 ContextPrecision / ContextRecall 误导，它俩管的是检索端。<code>AnswerRelevancy</code> 同属生成端但只管「切不切题」，答非所问才归它。",
            },
        ],
        "open": [
    ...
    "14-metric-result.html": {
        "mcq": [
            ...
            {
                "q": "把含 <code>MetricResult</code> 字段的 pydantic 行分别 <code>model_dump()</code> 和 <code>model_dump_json()</code>，两者对这个字段的处理一样吗？",
                "opts": [
                    "一样：两者都把它拆成 <code>{value, reason}</code> 字典",
                    "不一样：<code>model_dump()</code> 原样保留 <code>MetricResult</code> 对象，<code>model_dump_json()</code> 才走 <code>__json__</code> 拆成 <code>{value, reason}</code>",
                    "一样：两者都只取 <code>value</code>、丢掉 <code>reason</code>",
                    "不一样：<code>model_dump()</code> 会直接抛错，<code>MetricResult</code> 只能 JSON 序列化",
                ],
                "answer": 1,
                "why": "核心 schema 按 mode 分流：<code>model_dump()</code>（Python 口径）原样保留 <code>MetricResult</code> 对象——之后还能取 <code>.reason</code>；只有 <code>model_dump_json()</code>（mode=='json'）才调 <code>__json__()</code> 拆成 <code>{value, reason}</code> 字典再转成字符串。别想当然以为两者输出一致——一个留对象、一个出 JSON。",
            },
        ],
    },
    ...
    "17-answer-metrics-internals.html": {
        "mcq": [
            ...
            {
                "q": "AnswerRelevancy 的 <code>strictness</code> 会真的反推多次问题。照此推想，给 <code>AspectCritic</code> 设 <code>strictness=5</code> 是不是就让它判定 5 次再多数投票？",
                "opts": [
                    "是，5 次独立判定取多数，结论更稳",
                    "不会：<code>_ascore</code> 只 <code>generate</code> 一次，把单元素列表传进 <code>_compute_score</code>，那条 <code>Counter</code> 投票分支只在 1 个元素上跑，<code>strictness&gt;1</code> 实为 no-op",
                    "不会：<code>strictness&gt;1</code> 会直接报错，AspectCritic 只允许 strictness=1",
                    "会，但只在多轮样本上生效，单轮样本忽略该参数",
                ],
                "answer": 1,
                "why": "同名参数在同一课里行为不同，正是易混点。AnswerRelevancy 真的 <code>for _ in range(strictness)</code> 反推多次；而 AspectCritic 的 <code>_ascore</code> 只调一次 <code>generate</code>、再 <code>_compute_score([response])</code>——<code>Counter(...).most_common(1)</code> 作用在单元素列表上，结果就是那唯一的 verdict。构造时虽把 strictness 强制成奇数（像是为投票预留），但当前实现并未多次采样，所以 <code>strictness&gt;1</code> 不改变任何行为（ragas docstring 称其多数投票，属已知出入，读内部实现以代码为准）。",
            },
        ],
    },
        ...
        "mcq": [
            {
                ...
                "opts": [
                    ...
                    "它用当前输入在线微调（retrain）embedding 模型，让例题检索越用越准",
                    "它每次调用都把整个例题库重新算一遍 embedding，所以总能反映最新例题",
                    "它无视相似度、纯随机抽 k 条例题，以增加多样性",
                ],
                "answer": 0,
                "why": "动态版按输入用 <code>embed_query</code> + 余弦相似度挑 top-k（默认阈值 0.7），例题随题目变化更贴题。关键：它只把<strong>当前输入</strong>向量化去检索，例题向量是预先存好的——既不在线微调 embedding、也不每次重算全库，更不是随机抽；没 embedding 时（Simple 栈）才优雅退回最近 N 条。",
            },
            {
    ...
    "34-integrations-frameworks.html": {
        "mcq": [
            ...
            {
                "q": "<code>TestsetGenerator.from_langchain(llm, embedding_model)</code> 到底替你做了什么？",
                "opts": [
                    "把 LangChain 的 LLM / Embedding 包成 ragas 的 wrapper、装好一个生成器；喂文档、切块、建知识图谱是后续 <code>generate_with_langchain_docs</code> + transforms 的事",
                    "顺手接收你的 LangChain 文档、自动切块并建好知识图谱，省掉 transforms 那一步",
                    "强迫你放弃 LangChain，把 LLM 和数据全部改写成 ragas 原生对象",
                    "因为 ragas 只能读 LangChain / LlamaIndex 两家的数据，所以必须走这个入口",
                ],
                "answer": 0,
                "why": "<code>from_langchain</code> 只做适配：把 <code>lc_llm</code> / <code>lc_embeddings</code> 包成 <code>LangchainLLMWrapper</code> / <code>LangchainEmbeddingsWrapper</code> 并初始化一个（空的）KnowledgeGraph，让你在原生态里平滑接入。真正喂文档、切块、抽关系建图，是后一步 <code>generate_with_langchain_docs(docs, ...)</code> 配 transforms 完成的——别把「顺带建好图」的功劳记到 <code>from_langchain</code> 头上。",
            },
        ],
        "open": [
        ...
        "mcq": [
            {
                ...
                "answer": 3,
                "why": "关键区分：<code>fake_llm</code>（<code>EchoLLM</code>）把输入原样回显、<strong>完全确定</strong>，断言能写死；但 <code>fake_embedding</code>（<code>EchoEmbedding</code>）其实是 <code>np.random.rand(768)</code> 且<strong>没设种子</strong>，并不可复现——需要可复现向量的测试用的是另一个定种子的 <code>mock_embedding</code>（<code>np.random.seed(42)</code>）。fake 系列的共同价值是：不碰网络、不计费、不受真实模型采样波动，CI 才能稳定红绿。",
            },
            {
    ...
    "38-capstone.html": {
        "mcq": [
            ...
            {
                "q": "你用 <code>TestsetGenerator</code> 造好测试集、<code>to_evaluation_dataset()</code> 转成评测集，准备直接算 Faithfulness——为什么这一步通常得先把你的 RAG 再跑一遍？",
                "opts": [
                    "因为 <code>to_evaluation_dataset()</code> 会把字段打乱，必须重跑来恢复顺序",
                    "因为样本只有 <code>user_input</code> 和 <code>reference</code>，缺你系统的 <code>response</code> / <code>retrieved_contexts</code>",
                    "因为造题和评测必须用同一个 LLM 实例，重跑是为了对齐模型",
                    "因为不重跑，<code>reference</code> 就不会被引擎识别为标准答案",
                ],
                "answer": 1,
                "why": "生成器产出的是「黄金题」：每个 <code>eval_sample</code> 只带 <code>user_input</code>、<code>reference</code> 及它依据的 <code>reference_contexts</code>，并没有 <code>response</code>（你系统的回答）和 <code>retrieved_contexts</code>（你检索到的资料）。而 Faithfulness / AnswerRelevancy 恰恰要核对系统输出——所以得在 <code>@experiment</code> 里对每个 <code>user_input</code> 现跑一遍你的 RAG，补上 <code>response</code> / <code>retrieved_contexts</code> 再打分。这正是「造题」与「评测」两次生成的分工。",
            },
        ],
        "open": [
