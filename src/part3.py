...
<div class="card detail">
  ...
  两代指标的"门牌"都在 <span class="mono">src/ragas/metrics/__init__.py</span>：<span class="mono">__all__</span>（基类 + Simple 栈）、<span class="mono">_DEPRECATED_METRICS</span>（具名指标）、<span class="mono">__getattr__</span>、<span class="mono">_DEPRECATION_MESSAGE</span>——机制见上方手风琴，此处只列符号位置。具名指标的现代实现在 <span class="mono">src/ragas/metrics/collections/</span>。
...
<div class="card spark">
  ...
  <ul>
    ...
    <li><strong>公开面小而稳</strong>：顶层只稳定导出基类与 Simple 栈，具名指标集中到 collections——升级时不易误伤用户代码（旧路径如何平滑过渡到 v1.0，见上方手风琴）。</li>
  </ul>
</div>
...
注意 embedding 工厂从 <span class="mono">ragas.embeddings.base</span> 导入——从 <span class="mono">ragas.embeddings</span> 导 <span class="inline">embedding_factory</span> 已被标记废弃。
上面的 <span class="inline">await</span> 需放在 <span class="mono">async def</span> 里或用 <span class="mono">asyncio.run(...)</span> 执行（交互式 notebook 可直接 await）。</p>
...
<p>装饰器会按 <span class="inline">allowed_values</span> 的类型<strong>自动选指标种类</strong>（list → 离散、tuple / range → 数值、int → 排序）。函数返回的普通值会被自动包成 <span class="inline">MetricResult</span> 并按 allowed_values 校验。
...
<p>同样地，这些 <span class="inline">await</span> 需在 <span class="mono">async def</span> 或 <span class="mono">asyncio.run(...)</span> 中执行（notebook 可直接 await）。</p>
...
<div class="card key">
  ...
  <ul>
    ...
    <li>实战常<strong>两者混用</strong>最划算（具体取舍策略见上方手风琴）。</li>
  </ul>
</div>
"""
