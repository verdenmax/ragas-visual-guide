...
<div class="card analogy">
  <div class="tag">🧩 生活类比</div>
  把建图想成一条<strong>食材加工流水线</strong>：先把大块食材<strong>切小</strong>（splitter），再给每块<strong>贴标签</strong>——摘要、关键词、实体（extractor），
  然后把<strong>味道相近的摆到一起</strong>（relationship builder），最后<strong>挑掉不新鲜的</strong>（filter）。一道道工序下来，原料才变成能直接用的"半成品"。
</div>
...
<details class="accordion">
  ...
  <div class="acc-body">
    ...
    <div class="qa">
      ...
      <div class="a">默认分布里只放了 <span class="inline">SingleHopSpecificQuerySynthesizer</span>（单跳具体），多跳才分抽象 / 具体两种。单跳抽象不在默认题型里——当然你可以自定义 <span class="inline">QueryDistribution</span> 自行增减。</div>
    </div>
  </div>
</details>
