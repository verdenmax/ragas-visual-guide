...
<p><strong>本课最容易漏的一点</strong>：从文档到打分，中间其实有<strong>两次不同的生成</strong>——别把它们当成一步 👇</p>
<div class="flow">
  <div class="node"><div class="nt">你的文档</div><div class="nd">docs</div></div>
  <div class="arrow">→</div>
  <div class="node"><div class="nt">生成①：造黄金题</div><div class="nd">TestsetGenerator → 只有 user_input + reference</div></div>
  <div class="arrow">→</div>
  <div class="node"><div class="nt">生成②：现跑你的 RAG</div><div class="nd">补上 response + retrieved_contexts</div></div>
  <div class="arrow">→</div>
  <div class="node hl"><div class="nt">指标打分</div><div class="nd">两次生成合一后评测</div></div>
</div>
...
<h2>把零件接起来</h2>
<p>测试生成线的终点 <span class="inline">Testset</span> 装着<strong>问题 + 标准答案</strong>，正是两条主线的<strong>交汇处</strong>。但要注意：测试集只有 <span class="mono">user_input</span> + <span class="mono">reference</span>，真实的 <span class="mono">response</span> / <span class="mono">retrieved_contexts</span> 得<strong>现跑你的 RAG</strong> 才有。所以这份 golden 数据按两代入口分流：<span class="inline">to_evaluation_dataset()</span> 喂给经典 <span class="inline">evaluate()</span>；现代做法则把同一批"问题 + 标准答案"灌进一个带 backend 的 <span class="inline">Dataset</span>，在 <span class="inline">@experiment</span> 函数里边跑 RAG 边打分：</p>
...
<span class="kw">from</span> ragas <span class="kw">import</span> Dataset, experiment, evaluate
...
<span class="cm"># ③ Testset → EvaluationDataset（第 6、30 课）：喂经典 evaluate()，也用来灌下面的 golden</span>
...
<span class="cm"># golden：把测试集的"问题 + 标准答案"灌进带 backend 的 ragas Dataset（第 6、8 课）</span>
golden = Dataset(name=<span class="st">"rag_golden"</span>, backend=<span class="st">"local/csv"</span>, root_dir=<span class="st">"."</span>)
<span class="kw">for</span> s <span class="kw">in</span> eval_dataset:                                  <span class="cm"># 复用上面 ③ 的 EvaluationDataset</span>
    golden.append({<span class="st">"user_input"</span>: s.user_input, <span class="st">"reference"</span>: s.reference})
golden.save()
...
<div class="card spark">
  ...
  <ul>
    <li><strong>两条主线在 golden 处交汇</strong>：测试生成线（文档 → <span class="inline">Testset</span>）与评测线（指标 → 分数）在 Testset 的"问题 + 标准答案"处对接——经典路径用 <span class="inline">to_evaluation_dataset()</span> 接 <span class="inline">evaluate()</span>，现代路径把同一份 golden 灌进带 backend 的 <span class="inline">Dataset</span> 配 <span class="inline">@experiment</span>；全书前 37 课的能力在此汇成一条龙。</li>
...
<div class="card key">
  ...
  <ul>
    ...
    <li>记住交汇处：<span class="inline">generate_with_langchain_docs</span> 出 <span class="inline">Testset</span>（问题 + 标准答案）；经典路径 <span class="inline">to_evaluation_dataset()</span> → <span class="inline">evaluate()</span>，现代路径把同一份 golden 灌进 <span class="inline">Dataset</span> → <span class="inline">@experiment</span>；而 <span class="mono">response</span> / <span class="mono">retrieved_contexts</span> 要现跑你的 RAG 才有。</li>
