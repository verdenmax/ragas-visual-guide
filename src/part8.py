...
<span class="cm"># 经典路径（第 7 课）：先把 response/contexts 填好，再一次性打分</span>
<span class="cm"># result = evaluate(dataset=eval_dataset, metrics=[faith, crec, arel], llm=eval_llm)</span></pre>
  ...
  <div class="acc-body">
    ...
    <div class="qa">
      ...
      <div class="a">要"改 → 跑 → 比"的可持续迭代、每版留痕，用 <span class="inline">@experiment</span>（推荐，<a href="31-experiments-deep.html">第 31 课</a>）；
        只想对一份已填好的数据集<strong>快速算个总分</strong>，用经典 <span class="inline">evaluate()</span>（已废弃但仍可用，<a href="07-evaluate.html">第 7 课</a>）。两条入口共用同一套 collections 指标。</div>
    </div>
    <div class="qa">
...
<div class="card spark">
  ...
  <ul>
    ...
    <li><strong>统一接口、按需取舍</strong>：四件套都是 <span class="inline">await ascore(...)</span>；要留痕迭代用 <span class="inline">@experiment</span>，要快速总分用经典 <span class="inline">evaluate()</span>，同一套指标两种跑法。</li>
  </ul>
</div>
