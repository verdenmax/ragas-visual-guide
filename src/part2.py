...
<div class="card spark">
  ...
  <ul>
    <li><strong>to_dict 丢弃 None</strong>：样本"按需带字段"，同一个模型既能装齐全的 RAG 样本，也能只装"问题 + 回答"，喂给不同指标都不浪费列。</li>
    <li><strong>统一转换层</strong>：pandas / HuggingFace / CSV / JSONL 的互转都收敛到 <span class="inline">RagasDataset</span> 一处实现，子类无需各写一套（DRY）。</li>
    <li><strong>字段全 Optional</strong>：用一个数据模型覆盖多种评测场景，缺列在校验阶段才报错，定义阶段不强制。</li>
  </ul>
</div>
  ...
  <div class="acc-body">
    <div class="qa">
      ...
      <div class="a"><span class="inline">MultiTurnSample</span> 在 <span class="inline">user_input</span> 上挂了一个 <span class="inline">field_validator</span>，逐条检查：
        ...
        ③ <span class="inline">ToolMessage</span> 必须紧跟在"带 <span class="inline">tool_calls</span> 的 <span class="inline">AIMessage</span>"或"另一个 <span class="inline">ToolMessage</span>"之后。任一条不满足就抛 <span class="inline">ValueError</span>。</div>
    </div>
    <div class="qa">
...
<div class="card spark">
  ...
  <ul>
    ...
    <li><strong>调用与结果配对</strong>：<span class="inline">AIMessage.tool_calls</span> 描述"模型想调哪个工具"，随后的 <span class="inline">ToolMessage</span> 承接其结果，二者靠出现顺序配对，由 <span class="inline">field_validator</span> 保证合法。</li>
...
<details class="accordion">
  <summary><span class="badge-num">1</span> "local/csv" 这串字符串是怎么变成后端的？<span class="hint">点击展开详解</span></summary>
  <div class="acc-body">
    <div class="qa">
      <div class="q">🧪 _resolve_backend：字符串 → 后端实例</div>
      <div class="a">传字符串时，去全局 registry 查类再实例化；查不到就报出"可用后端清单"，传实例则原样返回：
<pre class="code"><span class="kw">if</span> isinstance(backend, str):
    registry = get_registry()
    backend_class = registry[backend]   <span class="cm"># 查不到 → KeyError → 列出可用后端</span>
    <span class="kw">return</span> backend_class(**kwargs)
<span class="cm"># 已经是 BaseBackend 实例则直接返回</span></pre>
      </div>
    </div>
    <div class="qa">
      <div class="q">✅ 用字符串当 backend 的好处</div>
      <div class="a">用户只需写 <span class="mono">"local/csv"</span>，无需手动 import 具体后端类；新后端注册进 registry 即可被字符串引用——<strong>插件式扩展</strong>，报错时还顺带列出所有可用项（backends 细节见 <a href="32-backends.html">第 32 课</a>）。</div>
    </div>
  </div>
</details>
...
<h2>列名对不齐怎么办</h2>
<p>若数据来自 <strong>HuggingFace Dataset</strong> 且列名和 ragas 默认不一致（如 <span class="mono">contexts_v1</span>），可传 <span class="mono">column_map={"contexts": "contexts_v1"}</span>；内部 <span class="inline">remap_column_names</span> 会先反向重命名再校验。注意：<strong>只有</strong>传入的是 HF <span class="inline">Dataset</span> 时才走这步，直接用 <span class="inline">EvaluationDataset</span> 则按字段名匹配。</p>

<div class="card warn">
  <div class="tag">⚠️ 已废弃，请转向 @experiment</div>
  从源码看，<span class="inline">evaluate()</span> 与 <span class="inline">aevaluate()</span> <strong>都会发出 <span class="mono">DeprecationWarning</span></strong>，提示改用 <span class="mono">@experiment</span> 装饰器。
  评测只是"打一次分"，而实验把"改动 → 跑分 → 留痕对比"固化成闭环。用法见 <a href="08-experiment.html">第 8 课</a>。
</div>
...
<details class="accordion">
  <summary><span class="badge-num">1</span> arun 内部的 5 个动作：取名 → 解析 backend → 建 Experiment → 并发跑 → 存盘 <span class="hint">点击展开详解</span></summary>
  <div class="acc-body">
    <div class="qa">
      <div class="q">🧪 五步拆解</div>
      <div class="a"><strong>①</strong> 没给 <span class="inline">name</span> 就用 <span class="inline">memorable_names.generate_unique_name()</span> 起个好记的名字（有 <span class="inline">name_prefix</span> 则拼前缀）；
        <strong>②</strong> backend = 传入的 / 装饰器默认的，否则沿用 <span class="inline">dataset.backend</span>，经 <span class="inline">Experiment._resolve_backend</span> 解析；
        <strong>③</strong> 建一个 <span class="inline">Experiment</span>（继承 <span class="inline">DataTable</span>，<a href="06-two-datasets.html">第 6 课</a>）；
        <strong>④</strong> 把每行包成任务，用 <span class="inline">asyncio.as_completed</span> 并发执行，<span class="inline">tqdm</span> 显示进度；
        <strong>⑤</strong> <span class="inline">save()</span> 落盘后返回该 <span class="inline">Experiment</span>。</div>
    </div>
    <div class="qa">
