...
即校验**内部链接无死链** + **HTML 结构 / 计数一致性**；并确保已重新运行 `build.py`、提交了生成的 `index.html` 与 `lessons/`。

### 本地导出 PDF

`print.html` 把全部 39 课合成单页文档（折叠卡片全部展开、自动分页），由 `build_print.py` 生成在**项目根目录**。用任意无头浏览器打 PDF，例如：

```bash
# 从项目根目录执行（print.html 由 build_print.py 生成在根目录）
cd src && python build_print.py && cd ..
